...
public class DASHPlayer implements Runnable{
	...
	public void run() {
		...
		Path plotLogFile = FileSystems.getDefault().getPath("all.data.dash.player-"+System.currentTimeMillis());
		...
		while(this.mpd.hasMoreSegments()){
			...
			try {
				...
				String s = (new Date(this.httpClient.getStartDownloadTime(numberOfSegmentsDownloaded))) + "\t"
